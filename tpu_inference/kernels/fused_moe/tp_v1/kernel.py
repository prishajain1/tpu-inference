...
def fused_tp_moe_from_routes(
    ...
    *,
    ...
    (packed_tokens, packed_weights, packed_slots, block_experts,
     actual_blocks) = packed

    @jax.shard_map(
        mesh=mesh,
        in_specs=(P(), P(None, None, tp_axis_name),
                  P(None, tp_axis_name, None), P(), P(), P(), P()),
        out_specs=P(),
        check_vma=False,
    )
    def run_local(packed_tokens, w1, w2, packed_weights, block_experts,
                  actual_blocks, packed_slots):
        packed_partial = _expert_major_pallas_call(
            packed_tokens,
            w1,
            w2,
            packed_weights,
        ...
        flat_partial = packed_partial.reshape(-1,
                                              tokens.shape[-1])[packed_slots]
    ...
    return run_local(packed_tokens, w1, w2, packed_weights, block_experts,
                     actual_blocks, packed_slots)
